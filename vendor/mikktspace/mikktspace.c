                   ...
                   const float fAngularThreshold)
{
    ...
    for (f = 0; f < iNrFaces; f++) {
        ...
        if (psTspace[index].iCounter == 0)	// tspace was not derived from a
        group
        {
        ...
        for (i = 0; i < verts; i++) {
            ...
            if (pContext->m_pInterface->m_setTSpaceBasic != NULL)
                pContext->m_pInterface->m_setTSpaceBasic(
                  pContext, tang, pTSpace->bOrient == TTRUE ? 1.0f : (-1.0f), f,
                  i);
        ...
        for (i = 0; i < 3; i++) {
            ...
            if ((pTriInfos[f].iFlag & GROUP_WITH_ANY) == 0
                ...
                if (neigh_indexL >= 0) // neighbor
                {
                    ...
                    const tbool bOrPre2
                      = (pTriInfos[neigh_indexL].iFlag & ORIENT_PRESERVING)
                            != 0 ?
                          TTRUE :
                          TFALSE;
                ...
                if (neigh_indexR >= 0) // neighbor
                {
                    ...
                    const tbool bOrPre2
                      = (pTriInfos[neigh_indexR].iFlag & ORIENT_PRESERVING)
                            != 0 ?
                          TTRUE :
                          TFALSE;
                    const tbool bDiff = bOrPre != bOrPre2 ? TTRUE : TFALSE;
                             ...
                             const SMikkTSpaceContext* pContext)
{
    ...
    ((void)(iUniqueTspaces)); /* quiet warnings in non debug mode */
